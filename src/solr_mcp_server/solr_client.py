...
class SOLRClient:
    ...
    def _initialize_connection(self) -> None:
        ...
        try:
            ...
            collection_url = urljoin(f"{self.config.base_url}/", f"{self.config.collection}/")
    ...
    def search(
        ...
        **kwargs: Any
        ...
        try:
            if rows is None:
                rows = min(self.config.max_rows, 100)  # Default to 100 if not specified
        ...
        try:
            ...
            for word, suggestion_data in spellcheck.get('suggestions', {}).items():
                if isinstance(suggestion_data, dict) and 'suggestion' in suggestion_data:
                    suggestions[word] = [s for s in suggestion_data['suggestion'][:count]]
