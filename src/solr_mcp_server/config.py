...
class SOLRConfig(BaseModel):
    ...
    collection: str = Field(
        description="Name of the SOLR collection to search"
    )
    username: Optional[str] = Field(
        default=None,
        description="Username for SOLR authentication"
    )
    password: Optional[str] = Field(
        default=None,
        description="Password for SOLR authentication"
    )
    timeout: int = Field(
        default=30,
    ...
    default_search_field: str = Field(
        default="text",
        description="Default field to search when no field is specified"
    )
    ...
    highlight_enabled: bool = Field(
        default=True,
        description="Whether to enable result highlighting by default"
    )
...
class Config(BaseModel):
    ...
    def from_env(cls, env_file: Optional[Union[str, Path]] = None) -> "Config":
        ...
        solr_config = SOLRConfig(
            ...
            highlight_enabled=os.getenv("SOLR_HIGHLIGHT_ENABLED", "true").lower() == "true",
        )
